...
class TelegramAuthentication:
    ...
    def verify_telegram_authentication(self, bot_token, request_data):
        """
        ...
        if not hmac.compare_digest(received_hash, _hash):
            raise NotTelegramDataError(
                'This is not a Telegram data. Hash from received authentication data does not match'
